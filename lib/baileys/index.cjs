...
const baileyCleanNumber$1 = (number, full = false) => {
    // No limpiar los números de grupo
    if (number.includes('@g.us')) {
        return number;  // Retorna el número tal como está si es de grupo
    }

    // Limpiar los números de teléfono privados
    number = number.replace('@s.whatsapp.net', '');
    return !full ? `${number}@s.whatsapp.net` : `${number}`;
};
...
const baileyIsValidNumber$1 = (rawNumber) => {
    const isGroup = rawNumber.includes('@g.us'); // Detectar mensajes de grupo
    const isPrivate = rawNumber.includes('@s.whatsapp.net'); // Detectar mensajes privados

    // Puedes decidir aquí si permites solo grupos, privados o ambos
    return isGroup || isPrivate; // Considerar válidos ambos tipos
};
    ...
    busEvents = () => [
        {
            ...
            func: ({ messages, type }) => {
                ...
                console.log('Emitiendo evento message:', payload);
